...
//
// Product photos are stored directly in the database as compressed data URLs
// (the admin page resizes them in the browser before saving), so no separate
// image storage service or payment method is needed.
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (path === 'settings') {
      ...
      if (method === 'PUT') return handleSettingsPut(request, env);
    }
...
const SETTINGS_KEYS = ['shop_name', 'default_phone', 'default_telegram', 'default_instagram'];

async function handleSettingsGet(env) {
  const { results } = await env.DB.prepare('SELECT key, value FROM settings').all();
  const obj = {};
  results.forEach(function (row) { obj[row.key] = row.value; });
...
async function listProducts(env) {
  const { results } = await env.DB.prepare(
    'SELECT id, name, price, description, image_url, phone, telegram, instagram FROM products ORDER BY created_at DESC'
  ).all();
...
async function createProduct(request, env) {
  ...
  const result = await env.DB.prepare(
    'INSERT INTO products (name, price, description, image_url, phone, telegram, instagram, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, unixepoch(), unixepoch())'
  ).bind(
    body.name, body.price, body.description || '', body.image_url || '',
    body.phone || '', body.telegram || '', body.instagram || ''
  ).run();
...
async function updateProduct(request, env, id) {
  ...
  await env.DB.prepare(
    'UPDATE products SET name = ?, price = ?, description = ?, image_url = ?, phone = ?, telegram = ?, instagram = ?, updated_at = unixepoch() WHERE id = ?'
  ).bind(
    body.name, body.price, body.description || '', body.image_url || '',
    body.phone || '', body.telegram || '', body.instagram || '', id
  ).run();
  return jsonResponse({ success: true });
}
